...
mysql --login-path="$OPS_META_LOGIN_PATH" -e "SOURCE $root_dir/sql/ddl.sql"
...
while IFS=$'\t' read -r instance_id login_path; do
  ...
  summary_out=""
  if ! summary_out=$(mysql --login-path="$login_path" --batch --raw -N -e "SET @instance_id:=$instance_id; SOURCE $root_dir/sql/collect/instance_logical_summary.sql" 2>&1); then
  ...
  if [[ -z "$instance_error" ]]; then
    if ! topn_out=$(mysql --login-path="$login_path" --batch --raw -N -e "SET @instance_id:=$instance_id; SOURCE $root_dir/sql/collect/top20_tables.sql" 2>&1); then
